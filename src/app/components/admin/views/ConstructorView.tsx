/* =============================================================================
   ConstructorView — Generador de proyectos Charlie
   Rol: asistente de configuración de módulos y frontstore para un tenant.
   Conecta al Supabase Charlie (config del shell) — nunca al Supabase del cliente.
   ============================================================================= */
...
import {
  ...
  ChevronDown, LogIn, Upload, Store,
  LayoutGrid, CreditCard, Download, Copy, CheckCheck,
  ExternalLink, Rocket, FileJson, Home,
...
// ─── Design tokens ────────────────────────────────────────────────────────────
// Todos los valores referencian CSS Custom Properties del sistema de theming.
// Ningún color hardcodeado — el tenant sobreescribe los tokens en runtime.
const C = {
...
const HUB_MODULE_IDS = new Set(MODULES.filter(m => m.label.toLowerCase().includes('hub')).map(m => m.id));
...
function SlimToggle({ on, onChange, disabled }: {
  on:        boolean;
  onChange:  () => void;
  disabled?: boolean;
}) {
...
function StepBar({ step, setStep, currentIdx }: {
  step:       string;
  setStep:    (s: any) => void;
  currentIdx: number;
}) {
          ...
          <React.Fragment key={s.id}>
            ...
            >
              ...
              <span style={{
                ...
                color: active ? C.gray900 : done ? C.gray500 : C.gray400,
                whiteSpace: 'nowrap',
              }}>
                {s.label}
              </span>
            ...
            {i < STEP_DEFS.length - 1 && (
              <div style={{
                ...
                backgroundColor: currentIdx > i ? C.orange : C.gray200,
                margin: '0 8px', flexShrink: 0, transition: 'background-color 0.3s',
              }} />
            )}
          </React.Fragment>
        );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: `1px solid ${C.gray200}`, flexShrink: 0, backgroundColor: value,
      }}>
        <input
      ...
      <span style={{
        fontSize: '0.72rem', fontFamily: 'monospace', color: C.gray500,
        backgroundColor: C.gray50, padding: '2px 6px', borderRadius: '4px',
...
export function ConstructorView({ onNavigate }: Props) {
  ...
  // Leer borrador guardado al montar
  useEffect(() => {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return;
    try {
      const d = JSON.parse(raw);
      setSavedMeta({ savedAt: d.savedAt, projectName: d.projectName ?? '', step: d.step ?? 'select', storeName: d.storeName ?? '' });
    } catch { /* borrador corrupto — ignorar */ }
  }, []);

  // Cerrar panel "Abrir" al hacer click fuera
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (loadPanelRef.current && !loadPanelRef.current.contains(e.target as Node)) setLoadOpen(false);
    };
    if (loadOpen) document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, [loadOpen]);

  // Cerrar dropdown de categoría al hacer scroll o click fuera
  useEffect(() => {
    const closePortal = () => { setOpenCat(null); setPortalPos(null); };

    const onMouseDown = (e: MouseEvent) => {
      const t = e.target as Node;
      const inPortal  = portalRef.current?.contains(t);
      const inHeaders = Array.from(headerRefs.current.values()).some(el => el.contains(t));
      if (!inPortal && !inHeaders) closePortal();
    };

    const onScroll = (e: Event) => {
      if (portalRef.current?.contains(e.target as Node)) return; // scroll interno → no cerrar
      closePortal();
    };
  ...
  const saveState = () => {
    const data = {
      savedAt: new Date().toISOString(), projectName, step, filterCat,
      selected: Array.from(selected),
      selectedSubs: Object.fromEntries(Object.entries(selectedSubs).map(([k, v]) => [k, Array.from(v)])),
      storeName, storeTagline, storeCurrency, primaryColor, secondaryColor, bgColor,
      homeSections, storefrontPages: sfPages,
    };
  ...
  const loadState = () => {
    ...
    try {
      ...
      setStep(d.step ?? 'select');
      setFilterCat(d.filterCat ?? 'Todos');
      ...
      setStoreCurrency(d.storeCurrency ?? 'USD');
      setPrimaryColor(d.primaryColor ?? '#FF6835');
      setSecondaryColor(d.secondaryColor ?? '#111111');
      setBgColor(d.bgColor ?? '#FFFFFF');
      setHomeSections(d.homeSections ?? DEFAULT_HOME_SECTIONS);
      setSfPages(d.storefrontPages ?? DEFAULT_STOREFRONT_PAGES);
      setLoadOpen(false);
  ...
  const toggle = (id: string) => {
    ...
    setSelected(prev => {
      ...
      if (n.has(id)) {
        n.delete(id);
        setExpanded(e => { const e2 = new Set(e); e2.delete(id); return e2; });
      } else {
        n.add(id);
        if (mod?.subOptions) setExpanded(e => new Set([...e, id]));
      }
      return n;
    });
  };
  ...
  const toggleSub = (mid: string, sid: string) =>
  ...
  const syncPagesWithModules = () => {
    setSfPages(prev => prev.map(p =>
      p.alwaysOn || (p.requiresModule && selected.has(p.requiresModule)) ? { ...p, enabled: true } : p
    ));
    setHomeSections(prev => prev.map(s =>
      s.requiresModule && selected.has(s.requiresModule) ? { ...s, enabled: true } : s
    ));
  };
  ...
  const loadTenant = async () => {
    ...
    try {
      ...
      MODULES.forEach(m => { if (modulosActivos.includes(m.id)) newSelected.add(m.id); });
  ...
  const handleApplyToTenant = async () => {
    ...
    setApplyResult(null);
    try {
      const modulosSeleccionados = MODULES.filter(m => selected.has(m.id) || m.required).map(m => m.id);
      ...
      console.error('[ConstructorView] Error aplicando tenant:', err);
  ...
  const allEnvVars = Array.from(new Set([
    ...BASE_ENV,
    'VITE_STORE_NAME', 'VITE_STORE_TAGLINE', 'VITE_CURRENCY', 'VITE_PRIMARY_COLOR', 'VITE_SECONDARY_COLOR',
    ...MODULES.filter(m => selected.has(m.id) || m.required).flatMap(m => {
      const base = m.subOptions ? [] : (m.envVars ?? []);
      const subs = m.subOptions
        ? m.subOptions.filter(s => (selectedSubs[m.id] ?? new Set()).has(s.id)).flatMap(s => s.envVars ?? [])
        : [];
      return [...base, ...subs];
    }),
  ]));
  ...
  const buildEnvContent = () => [
    '# .env.example — Charlie Marketplace Builder v1.5',
    `# Proyecto: ${projectName}`,
    `# Generado: ${new Date().toLocaleDateString('es-UY')}`,
    '',
    ...allEnvVars.map(v => {
      if (v === 'VITE_STORE_NAME')     return `${v}="${storeName}"`;
      if (v === 'VITE_STORE_TAGLINE')  return `${v}="${storeTagline}"`;
      if (v === 'VITE_CURRENCY')       return `${v}="${storeCurrency}"`;
      if (v === 'VITE_PRIMARY_COLOR')  return `${v}="${primaryColor}"`;
      if (v === 'VITE_SECONDARY_COLOR')return `${v}="${secondaryColor}"`;
      return `${v}=""`;
    }),
  ].join('\n');
  ...
  const buildConfigObj = () => ({
    ...
    generator: 'Charlie Marketplace Builder v1.5',
    modules:   MODULES.filter(m => selected.has(m.id) || m.required).map(m => m.id),
    providers: Object.fromEntries(
      Object.entries(selectedSubs).filter(([, s]) => s.size > 0).map(([k, v]) => [k, Array.from(v)])
    ),
    frontstore: {
      branding: { storeName, tagline: storeTagline, currency: storeCurrency, primaryColor, secondaryColor, bgColor },
      home:     { sections: homeSections.map(s => ({ id: s.id, enabled: s.enabled })) },
      pages:    sfPages.map(p => ({ id: p.id, path: p.path, enabled: p.enabled })),
    },
  });
  ...
  const copyText = (text: string, fn: (v: boolean) => void) => {
    navigator.clipboard.writeText(text).then(() => { fn(true); setTimeout(() => fn(false), 2200); }).catch(() => {});
  };
  ...
  const btnPrimary = (enabled = true): React.CSSProperties => ({
    display: 'inline-flex', alignItems: 'center', gap: '6px',
    padding: '8px 20px', borderRadius: '6px', border: 'none',
    backgroundColor: enabled ? C.orange : C.gray200,
    color:           enabled ? C.white  : C.gray400,
  ...
  const btnGhost: React.CSSProperties = {
    ...
    color: C.gray600, fontSize: '0.82rem', fontWeight: '500', cursor: 'pointer',
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', backgroundColor: C.gray50 }}>
      ...
      <div style={{ height: '48px', flexShrink: 0, backgroundColor: C.white, borderBottom: `1px solid ${C.gray200}`, display: 'flex', alignItems: 'center', padding: '0 24px', gap: '12px' }}>
        ...
        <span style={{ fontSize: '0.75rem', color: C.gray400 }}>
          {totalSelected} módulos · {allEnvVars.length} variables
        </span>
        <div style={{ flex: 1 }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', position: 'relative' }} ref={loadPanelRef}>
          <button
            ...
            style={{
              ...
              border: `1px solid ${saved ? C.green : C.gray200}`,
              backgroundColor: saved ? C.green10 : C.white,
              color: saved ? C.green : C.gray600,
          ...
          <button
            ...
            style={{
              display: 'inline-flex', alignItems: 'center', gap: '5px',
              padding: '5px 12px', borderRadius: '5px', position: 'relative',
              border: `1px solid ${loadOpen ? C.orange : C.gray200}`,
              backgroundColor: loadOpen ? C.orange10 : C.white,
              color: loadOpen ? C.orange : C.gray600,
              fontSize: '0.75rem', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s',
            }}
          >
            <FolderOpen size={12} /> Abrir
            {savedMeta && (
              <span style={{ position: 'absolute', top: '4px', right: '4px', width: '6px', height: '6px', borderRadius: '50%', backgroundColor: C.orange, border: `1.5px solid ${C.white}` }} />
            )}
          </button>
              ...
              {savedMeta ? (
                <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                    ...
                    <button
                      onClick={deleteDraft} title="Eliminar borrador"
                      style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', color: C.gray300, transition: 'color 0.1s' }}
        ...
        {step !== 'done' && (
          <>
            <span style={{ width: '1px', height: '16px', backgroundColor: C.gray200 }} />
            <button onClick={resetAll} style={{ fontSize: '0.72rem', color: C.gray400, background: 'none', border: 'none', cursor: 'pointer' }}>
              Resetear
            </button>
          </>
        )}
      ...
      {step === 'select' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          ...
          <div style={{ backgroundColor: C.white, borderBottom: `1px solid ${C.gray200}`, height: '42px', display: 'flex', alignItems: 'center', padding: '0 20px', gap: '14px', flexShrink: 0, position: 'sticky', top: 0, zIndex: 50 }}>
            ...
            <button
              onClick={() => setSelected(new Set(MODULES.filter(m => !m.required).map(m => m.id)))}
              style={{ fontSize: '0.72rem', color: C.orange, background: 'none', border: 'none', cursor: 'pointer', fontWeight: '600' }}
            >
              Todos
            </button>
            <button
              onClick={() => { setSelected(new Set()); setSelectedSubs({}); setExpanded(new Set()); }}
              style={{ fontSize: '0.72rem', color: C.gray500, background: 'none', border: 'none', cursor: 'pointer' }}
            >
              Limpiar
            </button>
          </div>
            ...
            {CATEGORIES.map(cat => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderRadius: '10px',
                    border: `1px solid ${isOpen ? catColor : C.gray200}`,
                    overflow: 'visible',
                    boxShadow: isOpen ? `0 0 0 1px ${catColor}20` : '0 1px 2px rgba(0,0,0,0.04)',
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: hubSel ? `${catColor}08` : C.white,
                      borderLeft: `3px solid ${hubSel ? catColor : C.gray200}`,
                      borderRadius: '8px', transition: 'all 0.12s',
                    }}
                  >
                    {/* Botón expand */}
                    <button
                      onClick={() => toggleCatPortal(cat)}
                      style={{ display: 'flex', alignItems: 'center', gap: '7px', flex: 1, background: 'none', border: 'none', cursor: 'pointer', padding: 0, minWidth: 0 }}
                    >
                      <ChevronRight
                        size={13} color={C.gray400}
                        style={{ flexShrink: 0, transition: 'transform 0.18s', transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}
                      />
                    ...
                    {hubMod && !hubMod.required && (
                      <div
                        ...
                        style={{
                          ...
                          flexShrink: 0, padding: '4px 6px', borderRadius: '5px',
                          backgroundColor: hubSel ? `${catColor}15` : 'transparent',
                          border: `1px solid ${hubSel ? catColor : C.gray200}`,
                        ...
                        {selectedRowMods.map(mod => (
                          <div
                            ...
                            style={{
                              ...
                              border: `1px solid ${catColor}40`, backgroundColor: `${catColor}12`,
                              color: catColor, fontSize: '0.6rem', fontWeight: '600',
                              cursor: mod.required ? 'default' : 'pointer',
                              flexShrink: 0, whiteSpace: 'nowrap', transition: 'opacity 0.1s',
                            }}
                          >
            ...
            return createPortal(
              <div
                ...
                style={{
                  ...
                  left: portalPos.left, width: portalPos.width,
                  zIndex: 9999, backgroundColor: C.white,
                  border: `1px solid ${pColor}`, borderRadius: '10px',
                  boxShadow: '0 16px 40px rgba(0,0,0,0.20)',
                  maxHeight: maxH, overflowX: 'hidden', overflowY: 'auto',
                }}
              >
                  ...
                  return (
                    <div key={mod.id}>
                      <div
                        ...
                        style={{
                          display: 'flex', alignItems: 'center', gap: '8px',
                          padding: '0 12px 0 22px', height: '32px',
                          borderLeft: `3px solid ${sel ? pColor : 'transparent'}`,
                          backgroundColor: sel ? `${pColor}07` : 'transparent',
                          cursor: mod.required ? 'default' : 'pointer',
                          borderBottom: isLast && !exp ? 'none' : `1px solid ${C.gray100}`,
                        ...
                        return (
                          <div style={{ borderLeft: `3px solid ${pColor}`, borderBottom: isLast ? 'none' : `1px solid ${C.gray100}`, backgroundColor: `${pColor}05`, padding: '8px 12px 10px 36px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '7px' }}>
                              <span style={{ fontSize: '0.6rem', fontWeight: '700', color: C.gray400, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                                Proveedores {mySubs.size}/{mod.subOptions!.length}
                              </span>
                              <button
                                onClick={() => mod.subOptions!.forEach(s => { const has = mySubs.has(s.id); if (allSel ? has : !has) toggleSub(mod.id, s.id); })}
                                style={{ fontSize: '0.65rem', color: pColor, background: 'none', border: 'none', cursor: 'pointer', fontWeight: '700', padding: 0 }}
                              >
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                              {mod.subOptions!.map(sub => {
                                ...
                                return (
                                  <button
                                    key={sub.id}
                                    onClick={() => toggleSub(mod.id, sub.id)}
                                    style={{
                                      display: 'inline-flex', alignItems: 'center', gap: '4px',
                                      padding: '3px 8px', borderRadius: '4px',
                                      border: `1px solid ${active ? pColor : C.gray200}`,
                                      backgroundColor: C.white,
                                      color: active ? pColor : C.gray500,
                                      fontSize: '0.7rem', fontWeight: active ? '600' : '400',
                                      cursor: 'pointer', transition: 'all 0.1s',
                                      boxShadow: active ? `0 0 0 1px ${pColor}` : 'none',
                                    }}
                                  >
          ...
          <div style={{ maxWidth: '760px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {MODULES.filter(m => selected.has(m.id) || m.required).map(mod => {
                  const subs = mod.subOptions
                    ? [...(selectedSubs[mod.id] ?? [])].map(sid => mod.subOptions!.find(s => s.id === sid)!).filter(Boolean)
                    : [];
                  return (
                    <div key={mod.id} style={{ borderRadius: '5px', border: `1px solid ${C.gray200}`, overflow: 'hidden' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 12px', backgroundColor: mod.required ? C.gray50 : C.orange10 }}>
                        <span style={{ fontSize: '0.78rem', fontWeight: '500', color: C.gray800, flex: 1 }}>{mod.label}</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '8px' }}>
              ...
              <button
                ...
                onClick={() => { if (projectName) { syncPagesWithModules(); setStep('frontstore'); } }}
                style={btnPrimary(!!projectName)}
              >
          ...
          <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ backgroundColor: C.white, border: `1px solid ${C.gray200}`, borderLeft: `3px solid ${C.orange}`, borderRadius: '6px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <p style={{ margin: 0, fontSize: '0.78rem', color: C.gray600 }}>
                El proyecto usará el <b style={{ color: C.gray900 }}>Frontstore nativo de Charlie</b>. La configuración se exporta en{' '}
                <code style={{ fontSize: '0.72rem', backgroundColor: C.gray100, padding: '1px 5px', borderRadius: '3px', color: C.gray700 }}>charlie.config.json</code> y en las variables de entorno.
              </p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>

              {/* Columna izquierda: Identidad + Paleta */}
              ...
              {/* Columna derecha: Páginas + Secciones Home */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>

                <div style={card}>
                  <div style={{ marginBottom: '14px' }}>
                    <p style={{ ...sectionTitle, margin: '0 0 2px' }}>Páginas del Storefront</p>
                    <p style={{ margin: 0, fontSize: '0.7rem', color: C.gray400 }}>Se activan según los módulos del Step 1</p>
                  </div>
                      ...
                      return (
                        <div key={page.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 10px', borderRadius: '5px', backgroundColor: page.enabled && !missing ? C.orange10 : C.gray50, border: `1px solid ${page.enabled && !missing ? `${C.orange}30` : C.gray100}`, opacity: missing ? 0.5 : 1 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ margin: 0, fontSize: '0.76rem', fontWeight: '500', color: C.gray800 }}>{page.label}</p>
                            <p style={{ margin: 0, fontSize: '0.63rem', color: C.gray400, fontFamily: 'monospace' }}>
                              {page.path}
                              {missing && <span style={{ color: C.amber, fontFamily: 'inherit', marginLeft: '4px' }}>· falta {page.requiresModule}</span>}
                            </p>
                          </div>
                          <SlimToggle
                            on={page.enabled && !missing}
                            onChange={() => setSfPages(prev => prev.map(p => p.id === page.id && !p.alwaysOn ? { ...p, enabled: !p.enabled } : p))}
                            disabled={!!page.alwaysOn || missing}
                          />
                        </div>
                      );
                    })}
                    ...
                    {homeSections.map((sec, idx) => {
                      ...
                      return (
                        <div key={sec.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 10px', borderRadius: '5px', backgroundColor: sec.enabled && !missing ? C.orange10 : C.gray50, border: `1px solid ${sec.enabled && !missing ? `${C.orange}30` : C.gray100}`, opacity: missing ? 0.5 : 1 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ margin: 0, fontSize: '0.76rem', fontWeight: '500', color: C.gray800 }}>{sec.label}</p>
                            <p style={{ margin: 0, fontSize: '0.63rem', color: C.gray400 }}>
                              {sec.description}
                              {missing && <span style={{ color: C.amber, marginLeft: '4px' }}>· falta {sec.requiresModule}</span>}
                            </p>
                          </div>
                          <SlimToggle
                            on={sec.enabled && !missing}
                            onChange={() => setHomeSections(prev => prev.map(s => s.id === sec.id ? { ...s, enabled: !s.enabled } : s))}
                            disabled={missing}
                          />
                        </div>
                      );
      ...
      {step === 'done' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          <div style={{ maxWidth: '880px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ backgroundColor: C.white, border: `1px solid ${C.gray200}`, borderRadius: '8px', padding: '28px 24px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'inline-flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'center' }}>
                {[
                  ...
                  <span key={i} style={{ fontSize: '0.72rem', fontWeight: '500', padding: '3px 10px', borderRadius: '20px', fontFamily: i === 0 ? 'monospace' : 'inherit', ...chip.style }}>
                    {chip.label}
                  </span>
                ))}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <div style={card}>
                ...
                <div style={{ ...codeBlock, maxHeight: '200px', marginBottom: '12px' }}>
                  ...
                  {allEnvVars.map(v => (
                    <p key={v} style={{ margin: '1px 0', color: 'var(--m-text-muted)', fontSize: '0.7rem' }}>
                      <span style={{ color: 'var(--m-info-bg)' }}>{v}</span>
                      <span style={{ color: 'var(--m-text-secondary)' }}>=</span>
                      <span style={{ color: 'var(--m-success-bg)' }}>
                        {v === 'VITE_STORE_NAME' ? `"${storeName}"` : v === 'VITE_CURRENCY' ? `"${storeCurrency}"` : v === 'VITE_PRIMARY_COLOR' ? `"${primaryColor}"` : v === 'VITE_SECONDARY_COLOR' ? `"${secondaryColor}"` : '""'}
                      </span>
                    </p>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '6px' }}>
                  ...
                  <button
                    onClick={() => copyText(buildEnvContent(), setCopied)}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', flex: 1, justifyContent: 'center', padding: '7px', borderRadius: '5px', border: `1px solid ${copied ? C.green : C.gray200}`, backgroundColor: copied ? C.green10 : C.white, color: copied ? C.green : C.gray600, fontSize: '0.76rem', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s' }}
                  >
              ...
              <div style={card}>
                ...
                <div style={{ ...codeBlock, maxHeight: '200px', marginBottom: '12px' }}>
                  {buildConfigJson().split('\n').map((line, i) => {
                    ...
                    return (
                      <p key={i} style={{ margin: 0, fontSize: '0.7rem', color: isBrace ? '#4B5563' : isStrVal ? '#7EE787' : 'var(--m-info-bg)', whiteSpace: 'pre', lineHeight: '1.6' }}>
                        {line}
                      </p>
                    );
                  })}
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  ...
                  <button
                    onClick={() => copyText(buildConfigJson(), setCopiedJson)}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', flex: 1, justifyContent: 'center', padding: '7px', borderRadius: '5px', border: `1px solid ${copiedJson ? C.green : C.gray200}`, backgroundColor: copiedJson ? C.green10 : C.white, color: copiedJson ? C.green : C.gray600, fontSize: '0.76rem', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s' }}
                  >
            ...
            {/* Aplicar módulos a tenant */}
            <div style={{ padding: '20px', backgroundColor: C.orange10, borderRadius: '12px', border: `1px solid ${C.orange20}` }}>
              <p style={{ margin: '0 0 6px', fontWeight: 700, fontSize: '0.875rem', color: C.orange }}>
                Aplicar módulos a un tenant
              </p>
              <p style={{ margin: '0 0 12px', fontSize: '0.8rem', color: C.gray500 }}>
                Actualiza <code style={{ fontFamily: 'monospace', fontSize: '0.78rem' }}>tenant_config.modulos</code> en Supabase Charlie para el tenant indicado.
              </p>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <input
                  ...
                  style={{ flex: 1, padding: '8px 12px', borderRadius: '8px', border: `1px solid ${C.gray200}`, fontSize: '0.875rem', outline: 'none', backgroundColor: C.white, color: C.gray900 }}
                />
                <button onClick={loadTenant} disabled={!tenantSlug.trim() || loadingTenant} style={btnGhost}>
                  {loadingTenant ? 'Cargando...' : '← Cargar'}
                </button>
                <button onClick={handleApplyToTenant} disabled={!tenantSlug.trim() || applyingTenant} style={btnPrimary(!!tenantSlug.trim() && !applyingTenant)}>
                  {applyingTenant ? 'Aplicando...' : 'Aplicar →'}
                </button>
              </div>
              {applyResult === 'ok'    && <p style={{ margin: '8px 0 0', fontSize: '0.8rem', color: C.green,  fontWeight: 600 }}>✅ Módulos aplicados correctamente a "{tenantSlug}"</p>}
              {applyResult === 'error' && <p style={{ margin: '8px 0 0', fontSize: '0.8rem', color: C.red,    fontWeight: 600 }}>❌ Error al aplicar. Verificá que el tenant existe.</p>}
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'space-between', paddingBottom: '8px' }}>
              ...
              <a
                href="https://github.com/CharlieUY711"
                target="_blank" rel="noopener noreferrer"
                style={{ ...btnGhost, textDecoration: 'none', backgroundColor: C.gray900, color: C.white, border: 'none' }}
              >
