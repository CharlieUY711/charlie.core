...
for fname in sorted(os.listdir(views_dir)):
    ...
    restantes = set(re.findall(r'#[0-9A-Fa-f]{3,6}\b', content))
    sin_mapear = [c for c in restantes if c not in COLOR_MAP]
    if sin_mapear:
        sin_mapear_global.update(sin_mapear)
    if reemplazados > 0:
        open(fpath, 'w', encoding='utf-8').write(content)
        total_archivos += 1
        total_reemplazos += reemplazados
        print(f'  {reemplazados:4d}  {fname}' + (f'  ⚠ sin mapear: {sin_mapear}' if sin_mapear else ''))

print(f'\nTotal: {total_archivos} archivos, {total_reemplazos} reemplazos')
if sin_mapear_global:
    print(f'Colores sin token en todo el proyecto: {sorted(sin_mapear_global)}')
else:
    print('Todos los colores mapeados correctamente.')
