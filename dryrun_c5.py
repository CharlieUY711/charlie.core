...
for hex_color in colores_ordenados:
    ...
    # Solo reemplazar como valor CSS entre comillas
...
restantes = list(set(re.findall(r'#[0-9A-Fa-f]{3,6}\b', content)))
restantes_no_mapeados = [c for c in restantes if c not in COLOR_MAP]

print(f"Reemplazados: {reemplazados}")
print(f"Colores restantes sin mapear: {len(restantes_no_mapeados)}")
for c in sorted(restantes_no_mapeados):
    print(f"  {c}")

# NO escribir — solo dry-run
print("\nDry-run OK — archivo NO modificado")
